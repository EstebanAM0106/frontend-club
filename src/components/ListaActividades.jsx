...
const ListaActividades = () => {
    ...
    const handleEditSubmit = async (e) => {
        ...
        try {
            ...
            setEventos(eventos.map((evento) => (evento.ID_Evento === editEvento.ID_Evento ? editEvento : evento)));
            setEditEvento(null);
        ...
        <div className="lista-eventos">
                ...
                {eventos.length > 0 ? (
                    eventos.map((evento) => (
                        <li key={evento.ID_Evento}>
                            <h3>{evento.Nombre}</h3>
                            <p>Fecha de Convocatoria: {formatDate(evento.Fecha_Convocatoria)}</p>
            ...
            {editEvento && (
                <form className="form-registro" onSubmit={handleEditSubmit}>
                    ...
                    <button type="button" onClick={() => setEditEvento(null)}>Cancelar</button>
                </form>
            )}
        </div>
    );
